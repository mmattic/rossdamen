if(typeof ctNocache.set_cookies_flag==='undefined'){ctNocache.set_cookies_flag=true;}
if(typeof ctNocache.ajaxurl==='undefined'){ctNocache.ajaxurl='/wp-admin/admin-ajax.php';}
...
{var ct_nocache_executed=true;var new_timestamp=Math.floor(Date.now()/1000);var old_timestamp=ct_getCookie('ct_timestamp');var checkjs_cookie=ct_getCookie('ct_checkjs');if(checkjs_cookie!=undefined)
{for(i=0;i<document.forms.length;i++)
{f=document.forms[i];for(j=0;j<f.elements.length;j++)
{e=f.elements[j];if(e.name!==undefined&&e.name.indexOf('ct_checkjs')!=-1)
{e.value=checkjs_cookie;}}}}
if((old_timestamp==undefined||new_timestamp-old_timestamp>86400||checkjs_cookie==undefined))
{ct_setCookie('ct_timestamp',new_timestamp);sendRequest(ctNocache.ajaxurl+'?'+Math.random(),ct_callback,'action=ct_get_cookie');}
...
{isVisitedMain=1;setTimeout(function(){document.cookie="ct_visited_main = 1; path = /;"},500);}
ct_visited_main=ct_getCookie('ct_visited_main');if(ct_visited_main==undefined&&isVisitedMain==-1)
{isVisitedMain=0;}
else
{isVisitedMain=1;}
cleantalk_user_info.is_main=isVisitedMain;setTimeout(function(){document.cookie="ct_user_info = "+escape(JSON.stringify(cleantalk_user_info))+"; path = /;"},500);}}