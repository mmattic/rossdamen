...
+function ($) {
  ...
  if (!$.fn.tooltip) throw new Error('Popover requires tooltip.js'/*tpa=http://www.mfa.org/sites/default/files/js/Popover requires tooltip.js*/)
...
+function ($) {
  ...
  Tab.prototype.show = function () {
    ...
    var hideEvent = $.Event('http://www.mfa.org/sites/default/files/js/hide.bs.tab', {
      relatedTarget: $this[0]
    })
    var showEvent = $.Event('http://www.mfa.org/sites/default/files/js/show.bs.tab', {
    ...
    this.activate($target, $target.parent(), function () {
      $previous.trigger({
        type: 'http://www.mfa.org/sites/default/files/js/hidden.bs.tab',
        relatedTarget: $this[0]
      })
      $this.trigger({
        type: 'http://www.mfa.org/sites/default/files/js/shown.bs.tab',
        relatedTarget: $previous[0]
      })
    })
  }
  ...
  function Plugin(option) {
    return this.each(function () {
      var $this = $(this)
      var data  = $this.data('http://www.mfa.org/sites/default/files/js/bs.tab')

      if (!data) $this.data('http://www.mfa.org/sites/default/files/js/bs.tab', (data = new Tab(this)))
      if (typeof option == 'string') data[option]()
    })
  }
...
(function ($) {
  ...
  // toggle button
  $('#navbar button.navbar-toggle').addClass('collapsed');
  $('#navbar button.navbar-toggle').click(function(event){
    // close all the slide menus
    $('#navbar #navbar-collapse .slide-menu').each(function( index ) {
    ...
    if($(this).hasClass('collapsed')) {
      var url = 'url("/themes/custom/mfa/images/icon.meun.close.ani2x.gif' + '?nocache=' + Math.random() + '")';
    } else {
      var url = 'url("/themes/custom/mfa/images/icon.meun.open.ani2x.gif' + '?nocache=' + Math.random() + '")';
    }
  ...
  // for all subnavs
  $('#navbar #navbar-collapse ul li.expanded').each(function( index ) {
    // add open menus on click
  ...
  // close buttons
  $('#navbar #navbar-collapse .slide-menu a.slide-close').each(function( index ) {
    $(this).click(function(event){
      $(this).closest('.slide-menu').removeClass('open');
      event.stopPropagation();
      event.preventDefault();
    });
  });

  // get top pos of collapsible region
  function getCollapsibleTop(){
    var position = $('#navbar').position();
    return position.top + $('#navbar .navbar-header').height();

  };

  // get scroll height
  function getScrollHeight(){
    height = $(window).height() - getCollapsibleTop();
    return height;
  };

  // set pos and padding for slide menus
  function setSlidePos(){
    $('#navbar #navbar-collapse .slide-menu').each(function( index ) {
      $(this).css('top', getCollapsibleTop());
      $(this).height(getScrollHeight());
    });
  }

  // set scroll height
  function setScrollHeight(){
    $('#navbar #navbar-collapse').each(function( index ) {
      $(this).height(getScrollHeight());
    });
  };
...
(function ($) {
  ...
  $('.field-credit-overlay').each(function( index ) {
    // for each label
    $(this).find('.field--name-field-credit .field--label').each(function( index ) {
      // on click
      $(this).click(function(event){
        $(this).closest('.field-credit-overlay').toggleClass('open');
        event.stopPropagation();
        event.preventDefault();
      });
    });
...
(function ($) {
  ...
  $('.carousel-pause').each(function( index ) {
    // get the carousel id
...
(function ($) {
  ...
  It's been modified into a function called at page load and then each time the
  page is resized. One large modification was to remove the set height before
  ...
  $('.equal-height-container').each(function(){
    equalheight($(this).find('.equal-height-element'));
  });
...
(function (_, $, Drupal, drupalSettings) {
  ...
  Bootstrap.option = function (key, value) {
    ...
    if (typeof key === "string") {
      ...
      // e.g., "http://www.mfa.org/sites/default/files/js/foo.bar" => { foo: { bar: true } }
      var parts = key.split('.');
      key = parts.shift();
      var obj = options;
      if (parts.length) {
...
(function ($) {
  ...
  // update ticket links based in on sale date
  function updateTnewTicketButton(){
    $('a[data-on-sale-date]').each(function(index){
      var onSale = new Date($(this).attr('data-on-sale-date'));
      // convert to UTC
      onSale = new Date(onSale.toUTCString());
      var now = new Date();
      // convert to UTC
      now = new Date(now.toUTCString());
      if(now.getTime() >= onSale.getTime()) {
        $(this).text($(this).attr('data-tickets-message')).removeClass('disabled').removeAttr('data-on-sale-date aria-disabled');

      }
    });
  };

  updateTnewTicketButton();
  // update tickets every second
  var tnewTicketButtonInterval = setInterval(updateTnewTicketButton, 1000);
